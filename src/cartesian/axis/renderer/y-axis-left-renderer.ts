...
export class YAxisLeftRenderer extends AxisRenderer {
  ...
  protected buildAxis(rendererConfig: AxisRendererConfig): SVGGAxisSelection {
    ...
    const d3Axis = axisLeft(rendererConfig.scale as AxisScale<number | string | Date>);
    d3Axis.ticks(5);
  ...
  protected styleAxis(rendererConfig: AxisRendererConfig): void {
    ...
    const ticksSelection = this.styleTicks(rendererConfig.axisSelection, rendererConfig.axisOption);

    ticksSelection.select<SVGElement>('line').attr('transform', 'translate(60,0)');
    ...
    ticksSelection.select<SVGElement>('text').attr('transform', 'translate(60,0)');
