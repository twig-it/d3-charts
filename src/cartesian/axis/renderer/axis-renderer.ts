...
import { CartesianAxisData, ScaleType, SVGChildSelection, SVGGAxisSelection, SVGGAxisTitleSelection } from '../../cartesian-object';
...
export abstract class AxisRenderer {
  protected constructor(protected readonly cartesianAxisScale: CartesianAxisScale) {
  }

  protected abstract buildAxis(rendererConfig: AxisRendererConfig): SVGGAxisSelection;
  ...
  protected buildAxisData(axis: Axis<string | number | Date>, scaleType: AxisScaleType): CartesianAxisData {
    return {
      axis: axis,
      scaleType: scaleType
    };
  ...
  protected styleAxisPath(axisSelection: SVGGAxisSelection, axisOption: AxisOptions): void {
    axisSelection
  ...
  protected styleTicks(axisSelection: SVGGAxisSelection, axisOption: AxisOptions): SVGChildSelection {
  ...
  protected styleTickText(axisSelection: SVGGAxisSelection, axisOption: AxisOptions): SVGChildSelection {
  ...
  protected styleTickLine(axisSelection: SVGGAxisSelection, axisOption: AxisOptions): void {
    axisSelection
  ...
  protected hideTickLine(axisSelection: SVGGAxisSelection): void {
    axisSelection.selectAll<SVGElement, {}>('.tick line').style('stroke-width', 0);
  }

  protected hideTickText(axisSelection: SVGGAxisSelection): void {
    axisSelection.selectAll<SVGElement, {}>('.tick text').style('stroke-width', 0);
  }

  protected setScaleFormatting(rendererConfig: AxisRendererConfig, axis: Axis<string | number | Date>): void {
    if (rendererConfig.axisOption.scale === AxisScaleType.Time) {
      ...
      axis.tickFormat(timeTickFormat as (tickLabel: string | number | Date) => string);
    }
  }


  public static getAxisSectionClass(isXAxis: boolean, isOpposite: boolean): string {
    if (isXAxis) {
      return !isOpposite ? `.${ChartLayoutClass.AxisBottom}` : `.${ChartLayoutClass.AxisTop}`;
    } else {
      return !isOpposite ? `.${ChartLayoutClass.AxisLeft}` : `.${ChartLayoutClass.AxisRight}`;
    }
  }
}
