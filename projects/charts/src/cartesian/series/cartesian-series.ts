...
import { CartesianAxisData, CartesianChartSelection } from '../cartesian-object';
...
export class CartesianSeries {
  ...
  public drawSeries(chartSelection: CartesianChartSelection, options: CartesianOptions): void {
    const plotSelection = chartSelection.select<SVGGElement>(`.${ChartLayoutClass.Plot}`);
    const seriesSection = plotSelection.select<SVGGElement>(`.${ChartLayoutClass.Series}`);

    this.updateScale(plotSelection, options);

    options.series.forEach((seriesOption, seriesIndex) => {
      const xAxisData = this.getAxisData(plotSelection, options.xAxis[seriesOption.xIndex], true, seriesOption.xIndex);
      const yAxisData = this.getAxisData(plotSelection, options.yAxis[seriesOption.yIndex], false, seriesOption.yIndex);
  ...
  public getAxisData(
    ...
    const axisData = this.cartesianAxis.getAxisData(plotSelection, axisOption, isXAxis, axisPosition);
    if (axisData) {
      return axisData;
    } else {
  ...
  private updateScale(plotSelection: CartesianChartSelection, options: CartesianOptions): void {
  ...
  private updateXScale(plotSelection: CartesianChartSelection, options: CartesianOptions): void {
    options.xAxis.forEach((axisOption, axisPosition) => {
      const combinedDomain = this.getXDomainForAllSeries(options.series, axisPosition, axisOption.scale);
      this.cartesianAxis.updateAxis(plotSelection, axisOption, true, axisPosition, combinedDomain);
    });
  }

  private updateYScale(plotSelection: CartesianChartSelection, options: CartesianOptions): void {
    options.yAxis.forEach((axisOption, axisPosition) => {
      const combinedDomain = this.getYDomainForAllSeries(options.series, axisPosition, axisOption.scale);
      this.cartesianAxis.updateAxis(plotSelection, axisOption, false, axisPosition, combinedDomain);
    });
  }
  ...
  private getXDomain(dataPoints: DataPoint[], scaleType: AxisScaleType): number[] | string[] {
    if (scaleType !== AxisScaleType.Category) {
  ...
  private getYDomain(dataPoints: DataPoint[] | OrdinalDataPoint[], scaleType: AxisScaleType): number[] | string[] {
    if (scaleType !== AxisScaleType.Category) {
      return [0, max<DataPoint, number>(dataPoints as DataPoint[], dataPoint => dataPoint.y)] as number[];
    } else {
