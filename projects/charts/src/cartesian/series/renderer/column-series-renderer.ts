...
export class ColumnSeriesRenderer extends SeriesRenderer {
  ...
  private drawSeriesForCategoryScale(config: SeriesRendererConfig): void {
    ...
    const colSelection = seriesSelection.selectAll<SVGRectElement, DataPoint>('rect').data(data);
  ...
  private drawSeriesForNumericScale(config: SeriesRendererConfig): void {
    ...
    colSelection
      ...
      .merge(colSelection)
      .style('fill', config.seriesOption.color)
      .attr('x', dataPoint => xScale(new Date(dataPoint.x as number - minColumnWidth / 2))!)
      .attr('y', dataPoint => yScale(dataPoint.y)!)
      .attr(
        'width',
        dataPoint =>
          xScale(new Date(dataPoint.x as number + minColumnWidth / 2))! - xScale(new Date(dataPoint.x as number - minColumnWidth / 2))! - 2
      )
      .attr('height', dataPoint => height - yScale(dataPoint.y)!)
      .each((_dataPoint, index, nodes) => {
        const thisSelection = select<SVGElement, DataPoint | OrdinalDataPoint>(nodes[index] as SVGElement);
  ...
  private getMinColumnWidth(dataPoints: DataPoint[]): number {
    const colMargin = 20;
    const dataSpan = extent<DataPoint, number>(dataPoints, dataPoint => dataPoint.x as number);
    const minData = dataSpan[0];
    const maxData = dataSpan[1];
    if (typeof minData !== 'undefined' && typeof maxData !== 'undefined') {
      const maxColumnWidth = (maxData - minData) / dataPoints.length - colMargin;
      const shiftedDataPoints = dataPoints.slice(1);
      const minXGranularity = min(shiftedDataPoints.map((dataPoint, index) => dataPoint.x as number - (dataPoints[index].x as number)));
